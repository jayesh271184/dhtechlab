...
echo "Hour,RA-GGSN,MED-GGSN,RA-PGW,MED-PGW,RA-SGW,MED-SGW,MED-MAHGGSN,MED-GUJGGSN,MED-RAJGGSN,MED-MPGGSN,TOTAL-RA,TOTAL-MED,DIFF-RA-MED,DIFF-MED-RAJ-MP">>$SPATH/OUTPUT-RECON.txt
#for (( i=00; i<=$HH; i++))
for i in {00..23}
do
if [ $HH == $i ]
...
HMAHGGSN=`cat /CGFBKUPEXT3/E2ERECON/D69*MED*${a}${b}${c}*.log /CGFBKUPEXT3/E2ERECON/D75*MED*${a}${b}${c}*.log | grep -v "I" | grep MAH |  awk -F ',' '{print$2}' | grep ${a}${b}${c}$i | sort | uniq | wc -l`
HGUJGGSN=`cat /CGFBKUPEXT3/E2ERECON/D69*MED*${a}${b}${c}*.log /CGFBKUPEXT3/E2ERECON/D75*MED*${a}${b}${c}*.log | grep -v "I" | grep GUJ |  awk -F ',' '{print$2}' | grep ${a}${b}${c}$i | sort | uniq | wc -l`
HRAJGGSN=`cat /CGFBKUPEXT3/E2ERECON/D69*MED*${a}${b}${c}*.log /CGFBKUPEXT3/E2ERECON/D75*MED*${a}${b}${c}*.log | grep -v "I" | grep RAJ |  awk -F ',' '{print$2}' | grep ${a}${b}${c}$i | sort | uniq | wc -l`
HMPGGSN=`cat /CGFBKUPEXT3/E2ERECON/D69*MED*${a}${b}${c}*.log /CGFBKUPEXT3/E2ERECON/D75*MED*${a}${b}${c}*.log | grep -v "I" | grep MPY |  awk -F ',' '{print$2}' | grep ${a}${b}${c}$i | sort | uniq | wc -l`
...
echo "$i,$HRAGGSN,$HMEDGGSN,$HRAPGW,$HMEDPGW,$HRASGW,$HMEDSGW,$HMAHGGSN,$HGUJGGSN,$HRAJGGSN,$HMPGGSN,$HRA,$HMED,$DIFFRAMED,$DIFFRAJMP">>$SPATH/OUTPUT-RECON.txt
