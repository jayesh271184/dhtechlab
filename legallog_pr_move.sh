...
DD=`date '+%c'`
NODE=CHN1
FDIR=/LVFMDUSAGEDATA/CHN1_MED_DATA/archieved/prepaid/
TDIR=/LVBACKLOGDATA/CHN1_MED_DATA/archieved/prepaid/
...
DUP=`cat ${LOG}${NODE}_PR_DUPLICATE_${FDAY}.log|grep ${i}|wc -l`
if [ $DUP -ge 1 ];then
echo "Duplicate file Found , ${i}" >> ${LOG}${NODE}_PR_DUPLICATE_${FDAY}.log
mv ${FDIR}${i} /u01/scripts/duplicate/;
else
echo "$CURDATE,$FDATE,$i,$ZZ,$SS"  >> ${LOG}${NODE}_legallog_pr_input_${FDAY}.log
